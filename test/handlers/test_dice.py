...
from diameter_learning.handlers import DiceCallback
...
def test_dice_callback():
    ...
    with mlflow.start_run():
        ...
        trainer.test(model)
        assert (
            artifact_path / 'DiceCallback' / 'dice_per_class.json'
            ).exists()
        with (artifact_path / 'DiceCallback' / 'dice_per_class.json').open(
            'r'
        ) as file_descriptor:
            dices = json.load(file_descriptor)
            assert dices.keys() == {'Lumen'}
            assert dices['Lumen'].keys() == {'mean', 'std', 'values'}
    mlflow.end_run()
